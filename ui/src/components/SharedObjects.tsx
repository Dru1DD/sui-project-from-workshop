...
export default function SharedObjects({
  ...
  const [isChangingPrice, setIsChangingPrice] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  const { data, isPending, error } = useSuiClientQuery(
    ...
    {
      ids:
        listedEvents?.data?.map(
          (event) => (event.parsedJson as any).list_hero_id,
        ) || [],
      options: {
        showContent: true,
        showType: true,
      ...
      {listedHeroes.length === 0 ? (
        ...
        <Grid columns="3" gap="4">
          {listedHeroes.map((obj) => {
            ...
            const level = Number(heroFields.level);
            const experience = Number(heroFields.experience);
            const power = Number(heroFields.power);
